...
export async function submitAppointment(formData: {
  ...
  try {
    ...
    const availability = await validateTimeSlotAvailability(
      formData.date,
      formData.time
    );
    ...
    const referenceId = `RDV-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 4)
      .toUpperCase()}`;
    ...
    const appointmentData: AppointmentData = {
      ...formData,
      location,
      //@ts-expect-error type mismatch, Timestamp is not recognized
