...
import { Calendar, Clock, User, Mail, Phone, Heart, MapPin, Loader2, RefreshCw } from "lucide-react"
...
export function BookingForm() {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.sessionType) {
      newErrors.sessionType = getTranslation("booking.validation.sessionType", language)
    }
        ...
        <div className="container mx-auto max-w-2xl">
          <Card className="text-center">
            <CardContent className="pt-8 pb-8">
              ...
              <h3 className="text-2xl font-bold mb-4 text-foreground">{getTranslation("booking.success", language)}</h3>
              {appointmentId && (
                <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground mb-6">
                  <MapPin className="w-4 h-4" />
                  <span>Reference ID: {appointmentId.slice(-8).toUpperCase()}</span>
                </div>
              )}
              <Button onClick={resetForm} variant="outline" className="flex items-center gap-2 bg-transparent">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className={errors.sessionType ? "border-destructive" : ""}>
                    <SelectValue placeholder={getTranslation("booking.sessionType.placeholder", language)} />
                  </SelectTrigger>
                  <SelectContent>
