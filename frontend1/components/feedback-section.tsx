...
export function FeedbackSection() {
      ...
      try {
        ...
        const sampleFeedbacks: Feedback[] = [
          ...
          },
          {
            ...
            name: "Marie L.",
            rating: 4,
            message: "Professional and caring approach. The sessions were very helpful for my anxiety management.",
            date: "2024-01-05",
            approved: true,
            language: "en",
          },
  ...
  const validateForm = () => {
    ...
    if (!formData.message.trim()) {
      newErrors.message = getTranslation("feedback.validation.message", language)
    }
    ...
    <section className="py-16 bg-gradient-to-br from-sage-50 to-ocean-50">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-charcoal mb-4">{getTranslation("feedback.title", language)}</h2>
          <p className="text-lg text-charcoal/70 max-w-2xl mx-auto">{getTranslation("feedback.subtitle", language)}</p>
        </div>
        ...
        <div className="text-center mb-12">
          <Button onClick={() => setShowForm(!showForm)} className="bg-ocean-600 hover:bg-ocean-700 text-white">
        ...
        {showForm && (
          <Card className="max-w-2xl mx-auto mb-12 border-sage-200">
            <CardHeader>
              <CardTitle className="text-charcoal">{getTranslation("feedback.form.title", language)}</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={getTranslation("feedback.form.name.placeholder", language)}
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder={getTranslation("feedback.form.message.placeholder", language)}
                    rows={4}
                ...
                <div className="flex gap-4">
                  <Button type="submit" disabled={isSubmitting} className="bg-ocean-600 hover:bg-ocean-700 text-white">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {feedbacks.length > 0 ? (
              feedbacks.map((feedback) => (
                <Card key={feedback.id} className="border-sage-200 hover:shadow-lg transition-shadow">
                  <CardContent className="p-6">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div>
                        <h4 className="font-semibold text-charcoal">{feedback.name}</h4>
                        <div className="flex items-center gap-2">
                          {renderStars(feedback.rating)}
                          <span className="text-sm text-charcoal/60">
                            {feedback.rating} {getTranslation("feedback.stars", language)}
                          </span>
                        </div>
                      </div>
                    </div>
                    <p className="text-charcoal/80 leading-relaxed">{feedback.message}</p>
                    <p className="text-sm text-charcoal/50 mt-4">{formatDate(feedback.date)}</p>
                  </CardContent>
                </Card>
              ))
            ) : (
              <div className="col-span-full text-center py-12">
                ...
                <p className="text-charcoal/60">{getTranslation("feedback.noFeedback", language)}</p>
              </div>
            )}
          </div>
        )}
